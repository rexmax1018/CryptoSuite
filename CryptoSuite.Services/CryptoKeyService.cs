...
public class CryptoKeyService : ICryptoKeyService
{
    ...
    /// <typeparam name="TModel">金鑰模型類型</typeparam>
    /// <param name="algorithm">加密演算法</param>
    /// <param name="filePath">儲存檔案完整路徑（可選）</param>
    /// <returns>產生結果（非同步）</returns>
    public async Task<KeyGenerationResult> GenerateAndSaveKeyAsync<TModel>(CryptoAlgorithmType algorithm, string? filePath = null) where TModel : class
    {
        var generator = _generatorFactory.Create<TModel>(algorithm);
        return await generator.GenerateAndSaveKeyAsync(filePath);
    }
    ...
    /// <param name="algorithm">加密演算法</param>
    /// <param name="path">檔案路徑</param>
    /// <returns>非同步載入結果</returns>
    public async Task<TModel> LoadFromFileAsync<TModel>(CryptoAlgorithmType algorithm, string path) where TModel : class
    {
        var loader = _loaderFactory.Create<TModel>(algorithm);
        return await loader.LoadFromFileAsync(path);
    }
